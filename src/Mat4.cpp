...
namespace math3D{
	Mat4::Mat4(){
		this->elements = vector<float>(16,0.0f);
	}

	Mat4::Mat4(float value){
		this->elements = vector<float>(16,value);
	}

	Mat4::Mat4(const float elements[16]){
		this->elements = vector<float>(elements,elements+16);
	}
	
	Mat4::Mat4(const Mat4& mat){
		this->elements = vector<float>(16);
	...
	unique_ptr<float[]> Mat4::getAsArray() const{
		auto res = unique_ptr<float[]>(new float[16]);
		for(int i = 0; i < 16 ; i++){
	...
	Mat4& Mat4::crossProduct(const Mat4& mat){
		...
		//unique_ptr<float[]> temp = unique_ptr<float[]>(new float[16]);
		auto temp = Mat4{};
