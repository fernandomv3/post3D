	...
	PhongMaterial::PhongMaterial():Material(){
		...
		this->setFragmentShaderSource(
	        ...
	        "void main(){\n"
	            ...
	            "vec4 normDirection;\n"
	            "vec4 difference;\n"
	            "vec4 attenLightIntensity;\n"
	            "for(int i=0; i< MAX_P_LIGHTS ;i++){\n"
	                "difference = pointLightPosition[i] - worldSpacePosition;\n"
	                "normDirection = normalize(difference);\n"
	                "attenLightIntensity = attenuateLight(pointLightColor[i],pointLightAttenuation[i],difference);\n"
	                "float cosAngIncidence;\n"
	                "float blinnPhongTerm = calculateBlinnPhongTerm(normDirection,normal,viewDirection,material.shininess,cosAngIncidence);\n"
					"\n"
