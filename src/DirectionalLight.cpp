...
DirectionalLight::DirectionalLight():Light(){
  this->target = std::shared_ptr<Object3D>(new Object3D());
  this->intensity = 1.0f;
}
...
DirectionalLight::DirectionalLight(DirectionalLight&& dLight):Light(dLight){
  this->target = std::move(dLight.target);
  this->intensity = dLight.intensity;
}
...
DirectionalLight& DirectionalLight::operator=(DirectionalLight&& dLight){
  Light::operator=(std::move(dLight));
  this->target = std::move(dLight.target);
  this->intensity = dLight.intensity;
  return *this;
}

std::shared_ptr<Object3D> DirectionalLight::getTarget()const{
  return this->target;
}

DirectionalLight& DirectionalLight::setTarget(std::shared_ptr<Object3D>obj){
  this->target = obj;
  return *this;
}
...
std::unique_ptr<float[]> DirectionalLight::getVectorToLightAsArray(const Camera& camera)const{
...
std::unique_ptr<DirLight> DirectionalLight::getAsStruct(const Camera& camera)const{
  std::unique_ptr<DirLight> light = std::unique_ptr<DirLight>{new DirLight};
...
std::shared_ptr<Mat4> DirectionalLight::getLightWorldMatrix() const{
  return this->lightWorldMatrix;
}

DirectionalLight& DirectionalLight::setLightWorldMatrix(std::shared_ptr<Mat4> lightWorldMatrix){
  this->lightWorldMatrix = lightWorldMatrix;
  return *this;
}
