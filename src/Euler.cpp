...
Euler::Euler(Euler&& euler){
  ...
  this->z = euler.z;
  this->order = euler.order;
  this->quaternion = std::move(euler.quaternion);
}
...
std::string Euler::getOrder()const{
  return this->order;
}
...
std::shared_ptr<Quaternion> Euler::getQuaternion()const{
  return this->quaternion.lock();
}

Euler& Euler::setQuaternion(std::shared_ptr<Quaternion> q){
  this->quaternion = std::weak_ptr<Quaternion>{q};
  return *this;
}
