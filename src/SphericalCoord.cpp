...
Vec3 orbit(const Vec3& origin, const Vec3& position, float deltaPhi, float deltaTheta, float radiusFactor){
  ...
  s.setR(fmax(0.2f,fmin(99.0f,r)));
  float phi = s.getPhi() + deltaPhi;
  phi = fmax(SphericalCoord::MINANG,fmin(SphericalCoord::MAXANG,phi));
  s.setPhi(fmax(SphericalCoord::EPS,fmin(SphericalCoord::MAXANG - SphericalCoord::EPS,phi)));
